...
function tagFromText(text) {
  const m = text.trim().match(/^@\s*(\w+)\s/);
  return m && TASK_TAGS.includes(m[1]) ? m[1] : null;
}

function emitDirect(payload) {
  if (globalThis._io) globalThis._io.of('/enjambre').emit('agent:direct', payload);
}
...
export default async function tasksRoutes(fastify) {
  fastify.post('/api/task', async (request, reply) => {
    ...
    const agent = tagFromText(trimmed);
    const cleanText = trimmed.replace(/^@\w+\s*/, '').trim() || trimmed;
    const db = getDb();
    const id = generateId();
    const created = now();
    const blockedBy = parseDependencies(trimmed);
    const initialStatus = blockedBy.length > 0 ? 'bloqueada' : 'pendiente';
    db.prepare(`INSERT INTO tasks (id, text, original_text, status, assigned_to, created, stage, stage_updated_at, blocked_by) VALUES (?, ?, ?, ?, ?, ?, 'backlog', ?, ?)`).run(id, cleanText, trimmed, initialStatus, agent, created, created, JSON.stringify(blockedBy));
    const task = db.prepare('SELECT * FROM tasks WHERE id = ?').get(id);
    fastify.log.info(`Task ${task.id} created for agent: ${agent} [${initialStatus}]`);
    emitDirect({ id:crypto.randomUUID(), from:'system', to:agent, text:cleanText, task_id:task.id, timestamp:now() });
